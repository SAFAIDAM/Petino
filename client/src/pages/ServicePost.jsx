...
function Posts() {
  ...
  const updateService = async (serviceId, updatedData) => {
    try {
      const response = await axios.put(`http://localhost:8000/api/services/${serviceId}`, updatedData);
  ...
  const toggleDescription = (serviceId) => {
    setServices(prevServices => {
      return prevServices.map(service => {
        if (service._id === serviceId) {
          return { ...service, showFullDescription: !service.showFullDescription };
        }
        return service;
      });
    });
  ...
  const filteredServices = Array.isArray(services) ? services.filter((service) => {
    return categoryFilter ? service.category.toLowerCase() === categoryFilter.toLowerCase() : true;
  }) : [];
...
<div className="items-center justify-between w-11/12 mb-6 md:flex md:mx-8 lg:mx-auto lg:px-8">
                ...
                <div className="flex items-center justify-center gap-4">
                    ...
                    <div className="w-[300px] h-[42px]  flex items-center gap-2 border border-[#2B2B2B] rounded-[30px] pl-4">
                        ...
                        <input className="pr-10 bg-transparent focus:outline-0" type="text" placeholder="Search" />
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between w-11/12 mx-auto mb-6 md:w-11/12 md:mx-8 lg:mx-auto lg:px-8">
                <h1 className="text-[20px] font-semibold text-[#6E6E6E] ">Recent services </h1>
                <div className="relative flex gap-5">
                    <select 
                    value={categoryFilter}
                    onChange={handleChange} className="flex items-center py-2 px-4 rounded-[30px] border text-[#E06C2E] border-[#E06C2E]">
                        <option className='text-center' value="">All</option>
                        <option className='text-center' value="Hosting">Hosting</option>
                        <option className='text-center' value="Grooming">Grooming</option>
        ...
        {filteredServices.map(service => (
          ...
          <div className="custom-shadow rounded-[25px] w-[320px] md:w-full py-6 px-2 mx-auto bg-white " key={service._id}>

            <div className='flex items-center justify-between mr-4'>
              <div className="flex items-center gap-4 mx-4">
              {/* User profile picture */}
              {service.userProfileImage ? (
                <img className="w-[50px] h-[50px] rounded-full" src={service.userProfileImage} alt="user picture" />

              ) : (
                <img className="w-[50px] h-[50px] rounded-full" src={imgDefault} alt="default user picture" />
              )}
              <p>{service.username}</p>

            </div>
 {/* Time elapsed since posting */}
              <span className="text-[15px] text-[#A6A6A6]">{formatTimeElapsed(service.datePosted)}</span>
            </div>
            ...
            <div className="mx-4 mt-5" style={{ maxWidth: '90%' }}>
              {service.showFullDescription ? (
                <p className="" style={{ maxWidth: '100%', wordWrap: 'break-word' }}>{service.description}</p>
              ) : (
                <p className="" style={{ maxWidth: '100%', wordWrap: 'break-word' }}>{service.description.slice(0, 100)}</p>
              )}
              {service.description.length > 100 && (
                <button className='font-extrabold' onClick={() => toggleDescription(service._id)}>
  ...
  <img className="h-80 max-sm:h-48 w-full rounded-[15px]" src={service.imageURL} alt="post image" />
  <div className="absolute px-6 py-1 text-sm bg-white border border-black rounded-full top-4 right-3">
    <h4 className="font-extrabold text-black">{service.category}</h4>
  </div>
</div>
            ...
            <div className="flex items-center justify-between mx-4 mb-2">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-500">
                  {new Date(service.startDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}
                  -
                  {new Date(service.dueDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}
                </p>
              ...
              <p  className="flex justify-end"><LiaStarSolid className='items-center' size={25} />{parseFloat(service.rating).toFixed(2)} </p>
              <RateService serviceId={service._id} />

                
              <button className={`rounded-3xl py-1 px-3 ${
    service.statut === 'not available' ? 'bg-red-500' : 'bg-green-300'
}`}>
  {service.statut}
</button>
                ...
                {currentUser && currentUser._id === service.userId && (
                  <div className="flex justify-end gap-3">
                    <img onClick={() => setEditedService(service)} src={editIcon} className="w-[19px] cursor-pointer" alt="edit icon" />  
                    <img onClick={() => deleteService(service._id)} src={deleteIcon} className="w-[22px] cursor-pointer" alt="delete icon"
    />            </div>
                )}
              </div>
            </div>
